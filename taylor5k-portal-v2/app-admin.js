...
function inviteBox(){if(!lastInvite)return '';const delivery=lastInvite.email_delivery||{},sent=delivery.sent===true,email=lastInvite.member?.email||'';return `<div class="invite-result"><div><span class="eyebrow">${sent?'SETUP EMAIL SENT':'MANUAL SETUP AVAILABLE'}</span><strong>${esc(lastInvite.member?.display_name||email||'Member')}</strong><p>${sent?`A setup email was sent automatically to ${esc(email)}.`:`The email could not be sent. Copy the setup link and code below.`} The code expires ${new Date(lastInvite.setup.expires_at).toLocaleString()}.</p></div><div class="invite-values"><label>Setup link<input readonly id="inviteLink" value="${esc(lastInvite.setup.setup_url)}"></label><label>8-digit code<input readonly id="inviteCode" value="${esc(lastInvite.setup.code)}"></label></div><div class="invite-buttons"><button class="button primary" id="copyInvite">Copy Link + Code</button><a class="button ghost" href="${esc(lastInvite.setup.setup_url)}" target="_blank">Open Setup Page ↗</a></div></div>`}
function superAdminPage(){
  ...
  return `<div class="page-layout super-center">
    ...
    ${inviteBox()}
    <section class="super-section"><div class="super-section-head"><div><h3>Add / Invite Member</h3><p>Creates or reactivates the member, generates a secure setup code, and emails the invitation automatically.</p></div></div><form id="addMemberForm" class="member-add-form"><label>Name<input name="display_name" required placeholder="Full name"></label><label>Email<input name="email" type="email" required placeholder="name@example.com"></label><label>Role<select name="role"><option value="viewer">Viewer</option><option value="editor">Editor</option><option value="admin">Admin</option><option value="super_admin">Super Admin</option></select></label><button class="button primary" type="submit">＋ Add & Email Member</button><span id="memberAddStatus"></span></form></section>
...
function bindSuperAdmin(){
  $('#addMemberForm')?.addEventListener('submit',async e=>{e.preventDefault();const fd=new FormData(e.currentTarget),status=$('#memberAddStatus');status.textContent='Creating and emailing invite…';try{const res=await superAction({action:'add_member',display_name:fd.get('display_name'),email:fd.get('email'),role:fd.get('role')});lastInvite=res;status.textContent=res.email_delivery?.sent?'Invite emailed ✓':'Member added — email failed';if(!DEMO)await loadRoleExtras();renderSuperAdminCenter()}catch(err){status.textContent=err.message}});
  $$('.member-card').forEach(card=>{const id=card.dataset.memberId;card.querySelector('[data-save-member]')?.addEventListener('click',async()=>{const btn=card.querySelector('[data-save-member]');btn.textContent='Saving…';try{await superAction({action:'update_member',member_id:id,display_name:card.querySelector('[data-member-name]').value,role:card.querySelector('[data-member-role]').value,active:card.querySelector('[data-member-active]').checked});await loadRoleExtras();renderSuperAdminCenter()}catch(err){alert(err.message);btn.textContent='Save'}});card.querySelector('[data-resend-member]')?.addEventListener('click',async()=>{const btn=card.querySelector('[data-resend-member]'),member=(data.team_members||[]).find(x=>String(x.id)===String(id));btn.disabled=true;btn.textContent='Sending…';try{const res=await superAction({action:'resend_invite',member_id:id});lastInvite={member:res.member||member,setup:res.setup,email_delivery:res.email_delivery};await loadRoleExtras();renderSuperAdminCenter()}catch(err){alert(err.message);btn.disabled=false;btn.textContent='Resend Invite Email'}});card.querySelector('[data-remove-member]')?.addEventListener('click',async()=>{const member=(data.team_members||[]).find(x=>String(x.id)===String(id));if(!confirm(`Remove ${member?.display_name||member?.email||'this member'} from the Taylor 5K portal? Their portal login will also be removed.`))return;try{await superAction({action:'remove_member',member_id:id});await loadRoleExtras();renderSuperAdminCenter()}catch(err){alert(err.message)}})});
